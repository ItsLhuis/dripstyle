...
>(config: VariantConfig<V>): MarkedVariantFunction<V, D, VariantStyleType<V>> {
  const cache = new Map<string, StyleValue>()

  const variantFunction: VariantFunction<V, D, StyleValue> = (props = {} as VariantProps<V, D>) => {
  ...
  return variantFunction as unknown as MarkedVariantFunction<V, D, VariantStyleType<V>>
}
