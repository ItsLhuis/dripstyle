...
/*
 * Entries are constrained to `FlatStyle | StyleFn`, so each one is contextually typed: objects and
 * inline-function returns get autocomplete and keep their literal values, while each entry's concrete
 * type is still inferred for `useStyles()`. Two invariants keep this working — do not break them:
 *   - `createVariant()` pins its return to `VariantStyleType<V>`; a free type parameter would be
 *     filled by the contextual `FlatStyle` and collapse variants to the wide style union.
 *   - the constraint stays a plain union, not a self-referential mapped type, which would stop
 *     rejecting invalid values. See variantReturn.test-d.ts and inlineStyles.test-d.ts.
 */
 ...
 *
 * Function entries inside a factory preserve their concrete return type, so a text style factory
 * remains assignable to `StyleProp<TextStyle>` and a view style factory remains assignable to
 * `StyleProp<ViewStyle>`.
 *
 * @param definition - A static style record or a factory function.
 * @returns The definition for later consumption by `useStyles()`.
 */
function createStyleSheet<T extends StyleSheetFactoryResult>(
  definition: (ctx: StyleSheetContext) => T
): StyleSheetFactory<(ctx: StyleSheetContext) => T>
function createStyleSheet<T extends StaticStyleSheetDefinition>(definition: T): StyleSheetFactory<T>
function createStyleSheet(
  definition: StyleSheetDefinition
): StyleSheetFactory<StyleSheetDefinition> {
  if (__DEV__) {
    warnIfNotConfigured("StyleSheet.create()")
  }

  if (typeof definition === "function") {
    return definition as StyleSheetFactory<StyleSheetDefinition>
  }

  return RNStyleSheet.create(
    definition as Record<string, Record<string, unknown>>
  ) as unknown as StyleSheetFactory<StyleSheetDefinition>
}
