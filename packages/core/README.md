...
Augmenting the `DripstyleThemes` and `DripstyleBreakpoints` interfaces enables full TypeScript
type-checking for theme properties, theme name literals, and breakpoint names across the entire
codebase.
...
## Inline Function Styles

A stylesheet entry can be a **function with your own arguments** that returns a style object. It
closes over `theme` and `runtime` from the factory, so you get per-instance styles with no wrapper
helper and full type inference:
...
const styles = StyleSheet.create(({ theme, runtime }) => ({
  ...
  // inline function entry — your own args, theme/runtime via closure
...
function Card({ isConnected, items }: { isConnected: boolean; items: unknown[] }) {
  const style = useStyles(styles)

  return (
    <View style={style.card(isConnected)}>
      <View style={style.content(items.length === 0)} />
    </View>
  )
}
```

- **No wrapper required.** Invalid style properties are flagged, and the full call signature is
  preserved — `style.card` stays `(selected: boolean) => ViewStyle`.
- **Fresh on theme/runtime change.** `useStyles()` re-runs the factory when the theme or runtime
  changes, so inline functions always close over the current values.
- **Passed through untouched.** Inline functions (and `createVariant()` results) are never invoked
  or registered with `RN.StyleSheet` during processing — only static object entries are.

### Optional: `viewStyle` / `textStyle` / `imageStyle`

Inline functions validate and infer on their own. If you _also_ want autocomplete and
property-pinpointed errors **inside** the returned object, wrap it in one of the optional identity
helpers — they return the object untouched and only add explicit typing:

```tsx
import { StyleSheet, viewStyle } from "@dripstyle/core"
...
These helpers are entirely optional — never required to use inline function styles.
